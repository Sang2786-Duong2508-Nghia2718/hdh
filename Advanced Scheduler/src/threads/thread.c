...
calc_priority (struct thread *t)
{
  ...
  int p = substract_y_from_x (substract_y_from_x (convert_n_to_fixed_point (PRI_MAX), divide_x_by_n (t->recent_cpu, 4)), convert_n_to_fixed_point (multiply_x_by_n (t->nice, 2)));
  p = convert_x_to_integer_zero (p);
...
{
  if (t == idle_thread) return;
  int recent_c = add_x_and_n (divide_x_by_y (multiply_x_by_y (multiply_x_by_n (2, load_avg), t->recent_cpu), multiply_x_by_n (2, load_avg) + convert_n_to_fixed_point (1)), t->nice);
  t->recent_cpu = recent_c;
}
...
calc_load_avg (void)
{
  ...
  load_avg = add_x_and_y (multiply_x_by_y (divide_x_by_y (59, 60), load_avg), divide_x_by_y (ready_threads, 60));
  ASSERT (load_avg >= 0)
}
